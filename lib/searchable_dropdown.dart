      ...
      final Widget? emplacedHint = _enabled
          ...
          : DropdownMenuItem<Widget>(
              child: prepareWidget(widget.disabledHint) ??
                  (prepareWidget(widget.hint)) ?? Container(width: 0, height: 0));
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            !widget.displayClearIcon
                ? SizedBox()
                : InkWell(
                    onTap: hasSelection && _enabled && widget.readOnly == false
                        ? () {
                            clearSelection();
                          }
                        : null,
...
class _DropdownDialogState<T> extends State<DropdownDialog> {
  ...
  void initState() {
    if (widget.searchFn != null) {
      ...
      Function matchFn;
      if (widget.isCaseSensitiveSearch ?? false) {
        ...
        ? new Container(
            ...
            child: Row(
                ...
                children: [
                  prepareWidget(widget.hint) ?? Container(width: 0, height: 0),
                  Column(
                    children: <Widget>[doneButtonWidget ?? Container(width: 0, height: 0), validatorOutputWidget],
                  ),
                ]),
          )
        : new Container(
